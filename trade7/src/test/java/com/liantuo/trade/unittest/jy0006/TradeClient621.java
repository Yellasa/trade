...
class Tast implements Runnable{
	...
	@Override
	public void run() {
		TradeRequest<TradePacketReqBodyInnerRefund> bizReq = new TradeRequest<TradePacketReqBodyInnerRefund>();
		...
		head.setRequest_code("0006_002_001");
		bizReq.setHead(head);
		bizReq.setBody(body);
		try {
	...
	public static void main(String[] args) {
		for (int i = 0; i < 2; i++) {
			newFixedThreadPool.submit( new TradeClient621().new Tast());
		}
		newFixedThreadPool.shutdown();
		
	}
	@Test
	public void test00066002001(){
		Tast task = new Tast();
		Tast task2 = new Tast();
		Tast task3 = new Tast();
		newFixedThreadPool.submit(task);
		newFixedThreadPool.submit(task2);
		newFixedThreadPool.submit(task3);
		newFixedThreadPool.shutdown();
