	...
	public void batchValidateParas(List<RealTimeAccountingVo> list){
		...
		for(int i =0 ;i <list.size();i++){
			...
			if(amountList.size() != 1){
				...
				throw new BusinessUncheckedException(ErrorCode002Constants.PARAS_CHECK_NOT_PASS,"所属业务交易请求编号："+list.get(i).getTradeStepNo() +"格式校验未通过;参数错误信息:请求参数中的四个金额中有且只能有一个");
			}
			...
			if(parasErrorInfo!=null && !parasErrorInfo.trim().equals("")){
				...
				throw new BusinessUncheckedException(ErrorCode002Constants.PARAS_CHECK_NOT_PASS,"所属业务交易请求编号："+list.get(i).getTradeStepNo() +"格式校验未通过;参数错误信息:" + parasErrorInfo);
			}
		}
	}
