...
@Component(value = "paymentPageQueryOuterServiceImpl")
public class PaymentPageQueryOuterServiceImpl implements com.liantuo.trade.bus.service.v1_1.BizTradePageQueryService<TradeRequest<QueryPage4SinlePaymentDetail>> {

	@Resource(name = "singlePaymentServiceImpl")
	 private SinglePaymentService singlePaymentService;

    @Override
    public Object queryByPage(TradeRequest<QueryPage4SinlePaymentDetail> req, TradeResponse tradeResponse) {
        TradePacketHead head = req.getHead();
        QueryPage4SinlePaymentDetail body = req.getBody();
