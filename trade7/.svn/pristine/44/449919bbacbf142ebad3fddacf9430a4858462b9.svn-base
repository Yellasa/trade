...
public class TradeEntryController {
    ...
    public TradeResponse excuteRequest(@RequestBody String requestXML, @PathVariable("request_code") String request_code, @PathVariable("version") String version) {
        return doTradeService(requestXML, request_code, version);
    }

    @SuppressWarnings({"rawtypes", "unchecked"})
    @ResponseBody
    @RequestMapping(value = "/{request_code}/{version}", method = RequestMethod.GET)
    public TradeResponse queryRequest(@RequestBody String requestXML, @PathVariable("request_code") String request_code, @PathVariable("version") String version) {
        return doTradeService(requestXML, request_code, version);
    }
     ...
     * @return
     */
    private TradeResponse doTradeService(String requestXML, String request_code, String version) {
        logger.info("请求报文：\n{}", requestXML);
