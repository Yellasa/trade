...
/**
 * @ClassName:     TradeAdjustLedgerInProcess.java
 * @Description:   0003_001_002 V1.0 CA台账入金
 * Company:联拓金融信息服务有限公司
 * @author         kelly
 * @version        V1.0  
 * @Date           2016-4-6 上午10:52:55 
 */
@JobFlow(value = "0003_001_002", version = "1.0", template = ATradeCreateSingleTxNoPaymentTemp.class)
public class TradeAdjustLedgerInProcess implements TradeCreateSingleTxNoPaymentInterface<TradePacketReqBodyAdjustLedgerIn> {

	private final Logger logger = LoggerFactory.getLogger(TradeAdjustLedgerInProcess.class);
	 ...
	 */
	@Override
	public void formatValidate(TradeRequest<TradePacketReqBodyAdjustLedgerIn> tradeRquest) throws Exception {
		String msg = TradeValidationUtil.validateRequest(tradeRquest);
		if (!StringUtils.isEmpty(msg)) {
			throw exceptionService.buildBusinessException("JY00030010021000100", msg);
		}
	}
	 ...
	 */
	@Override
	public void bizValidate(TradeRequest<TradePacketReqBodyAdjustLedgerIn> tradeRquest) throws Exception {
		// 交易编号重复校验
		checkDuplicate(tradeRquest);

		// 查询收到CA所属商户编号
		CreditAccount receiveca = bizAccountService.selectByAccountNo(tradeRquest.getBody().getReceive_account_no());
		if (null == receiveca) {
			throw exceptionService.buildBusinessException("JY00030010021000400");
		} else {
			receiveMerchantNo = receiveca.getMerchantNo();
		}
	}
	
	protected void checkDuplicate(TradeRequest<TradePacketReqBodyAdjustLedgerIn> tradeRquest) throws Exception {
		...
		if (!CollectionUtils.isEmpty(list)) {
			throw exceptionService.buildBusinessException("JY00030010021000300");
		}
	}
	 ...
	 */
	@Override
	public void tradeCreate(TradeRequest<TradePacketReqBodyAdjustLedgerIn> tradeRquest) throws Exception {
		try {
	...
	protected void tradeAdjustCreate(TradeRequest<TradePacketReqBodyAdjustLedgerIn> tradeRquest) throws Exception {
		...
		adjust.setRequestGroup(TradeConstants.TRADE_GROUP_LEDGER_IN);//请求组别03
	 ...
	 */
	@Override
	public void transaction(TradeRequest<TradePacketReqBodyAdjustLedgerIn> tradeRquest) throws Exception {
		try {
            // 创建生效业务台账，返回台账编号
	...
	protected String createLegder(TradeRequest<TradePacketReqBodyAdjustLedgerIn> req) {
        ...
        ledger.setReceiveAmount(AmountUtils.bizAmountConvertToLong(req.getBody().getAdjust_amount()));//收款金额
	...
	protected void freezeOp(TradeRequest<TradePacketReqBodyAdjustLedgerIn> tradeRquest) throws Exception {
		/** 实时记账操作 */
	...
	protected List<RealTimeAccountingRsp> account(TradeRequest<TradePacketReqBodyAdjustLedgerIn> tradeRquest) throws Exception {
		List<RealTimeAccountingRsp> realTimeAccountingRsps;
		RealTimeAccountVO accounvo = new RealTimeAccountVO();
		accounvo.setAccountNo(tradeRquest.getBody().getReceive_account_no());//收到CA账号
		...
		accounvo.setReservedFields1(tradeRquest.getBody().getReceive_account_extend_field_1());//收到CA账务备注1
		accounvo.setReservedFields2(tradeRquest.getBody().getReceive_account_extend_field_2());//收到CA账务备注2
		accounvo.setReservedFields3(tradeRquest.getBody().getReceive_account_extend_field_3());//收到CA账务备注3
		...
		accounvo.setRmk("");
		accounvo.setTradeReqCode(tradeRquest.getHead().getRequest_code());// 交易请求类型0003_001_002
		...
		realTimeAccountingRsps = bizAccountService.avlBalIncr(accounvo);//可用余额增加
		this.realTimeAccountingRsps = realTimeAccountingRsps;
		return realTimeAccountingRsps;
	}
	
	protected void updateStatus(TradeRequest<TradePacketReqBodyAdjustLedgerIn> tradeRquest) {
	 ...
	 */
	@Override
	public void tradeUpdateForFail(TradeRequest<TradePacketReqBodyAdjustLedgerIn> tradeRquest) throws Exception {
		// 实现更新交易为失败状态
		try {
	...
	protected void updateStatusFail(TradeRequest<TradePacketReqBodyAdjustLedgerIn> tradeRquest) {
		adjustAccount.setGmtLatestModified(new Date());//最后变更日期时间
